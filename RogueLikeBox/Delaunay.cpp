...
public:
	...
	{}
    bool ShouldRemove(const triangle& tri) const
	{
    ...
    inline bool operator() (triangle tri)
    {
        bool b = tri.IsLeftOf(m_itVertex);
...
bool triangle_test_pred(triangle tri)
{
    return true;
}

void Delaunay::Triangulate(const vertexSet& vertices, triangleSet& output) const
{
	...
	for (itVertex = vertices.begin(); itVertex != vertices.end(); itVertex++)
	{
        ...
        triangleIsCompleted triangleCompleted(itVertex, output, vSuper);
        
        for (tIterator tBegin = workset.begin() ; tBegin != workset.end() ; )
        {
            if (triangleCompleted.ShouldRemove(*tBegin))
            {
                tBegin = workset.erase(tBegin);
            }
            else
            {
                tBegin++;
        ...
        vertexIsInCircumCircle vertextInCircumCircle(itVertex, edges);
        
        for (tIterator tBegin = workset.begin() ; tBegin != workset.end() ; )
        {
            if (vertextInCircumCircle.ShouldRemove(*tBegin))
            {
                 tBegin = workset.erase(tBegin);
            }
            else{
